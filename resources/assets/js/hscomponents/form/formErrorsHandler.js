Project.Components.FormErrorsHandler = {
    ...
    showErrors: function(errors, form) {
        ...
        $.each(errors, function (name, messages) {
            ...
            formElement = form.find('#' + name + '_field');
            ...
            if (formElement[0]) {
                ...
                if (formElement[0].tagName.toUpperCase() == 'DIV') {
                    formElement.append('<p class="help-block alert alert-danger">' + messages[0] + '</p>');
                }
                else {
                    formElement.parent().addClass('has-error');
                    formElement.after('<p class="help-block alert alert-danger">' + messages[0] + '</p>');
                }
            }
        });
